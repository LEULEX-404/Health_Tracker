          ...
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
              <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--admin-text)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Stethoscope size={20} color="var(--p-cyan)" /> My Doctor Appointments
              </h3>
              <Link
                to="/Appointment"
                style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', padding: '0.6rem 1rem', borderRadius: '10px', textDecoration: 'none', background: 'rgba(0,180,216,0.1)', color: 'var(--p-cyan)', border: '1px solid rgba(0,180,216,0.25)', fontWeight: 600 }}
              >
                Manage All
              </Link>
            </div>
            ...
            {loading ? (
              <div style={{ minHeight: '220px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--admin-card-bg)', borderRadius: '16px', border: '1px solid var(--admin-border)' }}>
                <Loader2 className="spin" size={32} color="var(--p-cyan)" />
              </div>
            ) : doctorAppointments.length === 0 ? (
              <div style={{ padding: '2rem', background: 'var(--admin-card-bg)', borderRadius: '16px', border: '1px dashed var(--admin-border)', textAlign: 'center' }}>
                <p style={{ margin: 0, color: 'var(--admin-text-muted)' }}>No doctor appointments found.</p>
              </div>
            ) : (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: '1rem' }}>
                {doctorAppointments.map((apt) => (
                  <div key={apt._id} style={{ background: 'var(--admin-card-bg)', border: '1px solid var(--admin-border)', borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '0.9rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '0.75rem' }}>
                      <div>
                        <div style={{ color: 'var(--p-cyan)', fontWeight: 700, fontSize: '0.8rem', textTransform: 'uppercase' }}>{apt.date || '-'}</div>
                        <div style={{ color: 'var(--admin-text)', fontWeight: 700, fontSize: '1.05rem' }}>{apt.time || '-'}</div>
                      </div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                      <div
                        aria-hidden="true"
                        style={{ width: '46px', height: '46px', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,180,216,0.1)', border: '2px solid rgba(0,180,216,0.15)', color: 'var(--p-cyan)' }}
                      >
                        <Stethoscope size={20} />
                      </div>
                      <div>
                        <div style={{ color: 'var(--admin-text)', fontWeight: 600 }}>{apt.doctor || 'Doctor'}</div>
                        <div style={{ color: 'var(--admin-text-muted)', fontSize: '0.85rem' }}>{apt.specialty || 'Consultation'}</div>
                      </div>
                    </div>
                    <div style={{ height: '1px', background: 'var(--admin-border)' }} />
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem', color: 'var(--admin-text-muted)', fontSize: '0.9rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                        <MapPin size={14} color="var(--p-cyan)" />
                        <span>{apt.location || 'Location not specified'}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
                        <Clock size={14} color="var(--p-cyan)" />
                        <span>{apt.type || 'In Person'}</span>
                      </div>
                    </div>
                  </div>
                ))}
