...
import { getAllUsers, approveAppointment, rejectAppointment } from '../../utils/Imasha/adminApi';
import { Calendar, Stethoscope, HeartHandshake, Loader2, Clock, MapPin, User, ChevronRight, CheckCircle2, AlertCircle, Phone, Mail, XCircle } from 'lucide-react';
...
export default function AdminAppointmentsTab() {
  ...
  useEffect(() => {
    ...
    if (activeTab === 'caregiver') {
      const fetchData = async () => {
        ...
        try {
          // Fetch Bookings
          const res = await fetch(`${import.meta.env.VITE_API_URL}/tharindu/bookings/admin/all`, {
            headers: {
              'Content-Type': 'application/json',
              Authorization: `Bearer ${token}`
            }
  ...
  const renderStatus = (status) => {
    switch (status?.toLowerCase()) {
      ...
      case 'confirmed':
        return <span className="status-pill success"><CheckCircle2 size={12} style={{ marginRight: '4px' }} /> Confirmed</span>;
      case 'rejected':
      case 'cancelled':
  ...
  const filteredDoctorAppointments = doctorAppointments.filter((apt) => {
  ...
  const handleApproveDoctor = async (appointmentId) => {
    setUpdatingDoctorId(appointmentId);
    try {
      const result = await approveAppointment(token, appointmentId);
      if (result?.emailSent === false) {
        toast.success(`Doctor appointment confirmed. Email failed${result?.emailError ? `: ${result.emailError}` : '.'}`);
      } else {
        toast.success('Doctor appointment confirmed and email sent.');
      }
      fetchDoctorAppointments();
    } catch (e) {
      toast.error(e?.response?.data?.message || 'Failed to confirm doctor appointment.');
    } finally {
      setUpdatingDoctorId('');
    }
  };

  const handleCancelDoctor = async (appointmentId) => {
    const ok = window.confirm('Do you want to cancel this doctor appointment?');
    if (!ok) return;
    ...
    try {
      ...
      if (result?.emailSent === false) {
        toast.success(`Doctor appointment cancelled. Email failed${result?.emailError ? `: ${result.emailError}` : '.'}`);
      } else {
        toast.success('Doctor appointment cancelled and email sent.');
      }
      fetchDoctorAppointments();
    } catch (e) {
      toast.error(e?.response?.data?.message || 'Failed to cancel doctor appointment.');
    } finally {
      setUpdatingDoctorId('');
    }
  };
          ...
          {activeTab === 'doctor' && (
            ...
            >
              <h3 style={{ margin: 0, fontSize: '1.2rem', color: 'var(--admin-text)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <Stethoscope size={18} color="var(--p-cyan)" />
              ...
              <div style={{ display: 'flex', flex: 1, minWidth: '250px', background: 'var(--admin-card-bg)', border: '1px solid var(--admin-border)', borderRadius: '10px', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', padding: '0 0.85rem', color: 'var(--admin-text-muted)' }}>
                  <Stethoscope size={14} />
                </div>
                <input
                  type="text"
                  placeholder="Search doctor appointments..."
                  value={doctorSearchTerm}
                  onChange={(e) => setDoctorSearchTerm(e.target.value)}
                  style={{ flex: 1, background: 'transparent', border: 'none', padding: '0.75rem 1rem 0.75rem 0', color: 'var(--admin-text)', outline: 'none' }}
                />
              </div>

              {loading ? (
                <div className="loading-state" style={{ height: '320px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px solid var(--admin-border)' }}>
                ...
                <div className="empty-state" style={{ padding: '3rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'rgba(255,255,255,0.02)', borderRadius: '16px', border: '1px dashed rgba(255,255,255,0.1)' }}>
                  <div style={{ opacity: 0.5, marginBottom: '1rem' }}><Stethoscope size={48} /></div>
                  <p style={{ margin: 0, color: 'var(--admin-text)' }}>No doctor appointments found.</p>
                </div>
              ) : (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  {filteredDoctorAppointments.map((apt) => (
                    ...
                    >
                      <div style={{ background: 'rgba(0,180,216,0.08)', padding: '0.85rem', borderRadius: '12px', textAlign: 'center', minWidth: '88px' }}>
                        <div style={{ fontSize: '0.75rem', color: 'var(--p-cyan)', fontWeight: 700, textTransform: 'uppercase' }}>
                          {apt.date || '-'}
                        </div>
                        <div style={{ fontSize: '1rem', color: 'var(--admin-text)', fontWeight: 800, lineHeight: 1.2, marginTop: '0.35rem' }}>
                          {apt.time || '-'}
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.45rem' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                          <Stethoscope size={14} color="var(--p-cyan)" />
                          <span style={{ fontSize: '0.85rem', color: 'var(--admin-text-muted)', minWidth: '60px' }}>Doctor:</span>
                          <span style={{ fontSize: '0.95rem', color: 'var(--admin-text)', fontWeight: 600 }}>
                            {apt.doctor || 'Doctor'}
                          </span>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap', marginTop: '0.2rem', color: 'var(--admin-text-muted)', fontSize: '0.85rem' }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><Mail size={12} /> {apt.patientEmail || '-'}</span>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.35rem' }}><Phone size={12} /> {apt.patientPhone || '-'}</span>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.5rem' }}>
                        ...
                        {apt.status === 'Pending' ? (
                          <div style={{ display: 'flex', gap: '0.5rem' }}>
                            <button
                              type="button"
                              onClick={() => handleApproveDoctor(apt._id)}
                              disabled={updatingDoctorId === apt._id}
                              style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1px solid rgba(16,185,129,0.25)', background: 'rgba(16,185,129,0.12)', color: '#10b981', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: updatingDoctorId === apt._id ? 0.6 : 1 }}
                            ...
                            <button
                              type="button"
                              onClick={() => handleCancelDoctor(apt._id)}
                              disabled={updatingDoctorId === apt._id}
                              style={{ width: '36px', height: '36px', borderRadius: '10px', border: '1px solid rgba(239,68,68,0.25)', background: 'rgba(239,68,68,0.12)', color: '#ef4444', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', opacity: updatingDoctorId === apt._id ? 0.6 : 1 }}
